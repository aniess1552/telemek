...
module.exports = {
  ...
  register: (bot) => {
    bot.command("backup", ownerOnly, async (ctx) => {
      const loadingMessage = await ctx.reply("📦 Membuat backup, mohon tunggu...");
      ...
      output.on("close", async () => {
        try {
          ...
          fs.unlinkSync(outputPath); // Hapus file ZIP setelah dikirim
        } catch (err) {
          console.error("Gagal mengirim file:", err.message);
          ctx.reply("❌ Gagal mengirim file backup.");
        }
      });
